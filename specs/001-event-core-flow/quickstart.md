...
## Admin Recovery Local Implementation Record (2026-08-30)
...
- 로컬에서 타입 검사, ESLint, 단위 테스트 17건, Next.js 운영 빌드는 통과했다. 관리자
  목록은 닉네임, 캐릭터, 제출 여부, 입장 시각만 반환하며 답변 본문은 조회하지 않는다.
- 이 PC의 Docker 환경에서는 격리 PostgreSQL과 브라우저 시나리오를 실행하지 않았다.
  따라서 Scenario 4·5의 실제 DB 통합 테스트와 배포 앱 360px 검증은 아직 남아 있고,
  T060은 그 검증이 끝날 때까지 완료 처리하지 않는다.
