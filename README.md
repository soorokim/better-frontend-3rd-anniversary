...
이번 버전부터 관리자는 질문을 최대 네 개까지 순서대로 준비하고, 참가자는 현재 열려 있는 질문에 답한 뒤 이전 질문 답변을 `/answers`에서 다시 고칠 수 있다. 진행자 화면에서는 각 답변을 **공개됨** 또는 **제외됨**으로 처리해 다음 후보에서 빼고, 질문을 모두 마치면 다음 질문으로 넘어갈 수 있다. 배포 뒤에는 관리자 화면에서 질문 순서와 참가자 답변 목록이 보이는지, 발표 화면에서 다음 질문으로 넘어가는지 한 번 확인한다.
