...
export const invitationSchema = z.object({ inviteCode: z.string().min(4) }).strict();
export const resetCodeSchema = z.string().regex(/^\d{8}$/, '초기화 코드는 숫자 8자리여야 합니다.');
export const registerSchema = z.object({
  inviteCode: z.string().min(4), nickname: nicknameSchema, pin: pinSchema, pinConfirmation: pinSchema,
...
export const completePinResetSchema = z.object({
  inviteCode: z.string().min(4), nickname: nicknameSchema, resetCode: resetCodeSchema,
